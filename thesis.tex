...
generates metrics for the ease of extraction, and the refrequency of
execution.  The ease of extraction metric is mostly a reflection of
the nubmer of potental conflicts.  The frequency of exectuion value
can help the user focus parallelization efforts only highly
exectuined, or hot, regions of code.  Loops created by \textit{for} or
\textit{while} constructs are often the source of hot code
regions~\cite{bala:2000:pldi}.  However, a loop may contain an
expesive operation, such as memory allocation or file IO, and may not
be executied frequelty.  Furthermore, a loop may contain instructions
that are worthly of paralelization, but are included in a loop that
also may conflict with another parrallel task.  Conflicts make tasks
difficult to parallelize; some tools and may reduce the value of a
potential parallel task with conflicts, or elimiate such tasks
